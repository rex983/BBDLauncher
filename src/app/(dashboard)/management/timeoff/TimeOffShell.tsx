...
// The parent page.tsx server component hydrates this shell with the
// initial queue rows (pending + approved + denied) so first paint is
// populated. Client refetches on decision / edit / role-preview change.
...
export default function TimeOffShell({
  initialQueueRows,
}: {
  initialQueueRows: Row[];
}) {
...
function splitBuckets(rows: Row[]): { p: Row[]; a: Row[]; d: Row[] } {
  const p: Row[] = [];
  const a: Row[] = [];
  const d: Row[] = [];
  for (const r of rows) {
...
function RequestsQueue({
  ...
  const [denied, setDenied] = useState<Row[]>(initialSplit.d);
  ...
  // Skip the initial refetch — initialRows already covers the viewer's
  // scope. First real load fires when the admin flips viewAsOffice or
  // after a decision / edit.
  ...
  const load = useCallback(async () => {
    ...
    const { p: p2, a, d } = splitBuckets(rows);
    setPending(p2);
    setApproved(a);
    setDenied(d);
    setLoading(false);
