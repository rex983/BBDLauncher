...
// The page.tsx server component hydrates this shell with initialRows for
// the viewer's default scope. The first client-side refetch only fires
// when the admin flips office/department filters; the tick timer
// re-computes durations locally without a network hop.
...
export default function TimesheetsShell({ initialRows }: { initialRows: Row[] }) {
  ...
  // Skip the initial refetch — initialRows already reflects the viewer's
  // default scope. First real fetch only happens when the admin flips a
  // filter (or when the role-preview flips office).
