...
export async function GET(
  ...
  const { data: row, error } = await supabase
    ...
    .select(
      "id, employee_profile_id, reporter_profile_id, title, severity, category, status, occurred_at, description, document, acknowledgement_text, ai_provider, ai_model, ai_prompt, ai_generated_document, attachments, manager_signed_at, manager_signature_text, employee_signed_at, employee_signature_text, cancelled_at, cancelled_reason, created_at, updated_at",
    )
    .eq("id", id)
    .single();
  if (error || !row) {
