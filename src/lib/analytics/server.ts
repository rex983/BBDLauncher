...
export async function getLauncherAnalytics(
  ...
  const shape = (m: Map<string, DestAgg>): AnalyticsDestStat[] =>
    [...m.values()]
      .map((a) => ({
        id: a.id,
        name: a.name,
  ...
  const usersStats: AnalyticsUserStat[] = [...perUser.values()]
    .map((u) => {
  ...
  const recent: AnalyticsRecentEvent[] = events.slice(0, 50).map((e) => {
