...
// Manager/admin composes and publishes a memo. Handles all three
// acknowledgement modes and four audience scopes. Publish mode is
// binary: publish now, or save as draft for later manual publish.
// Attachment upload UX mirrors FileIncidentDialog.
export function ComposeMemoDialog({
  ...
  // Fetch employee roster once when the picker becomes relevant (custom
  // scope). Uses the same scope-filtered endpoint as the incident dialog.
  useEffect(() => {
    if (!open || audienceScope !== "custom") return;
    let cancelled = false;
    (async () => {
  ...
  const reset = () => {
    ...
    setAckMode("read_receipt");
    setAudienceScope(defaultScope);
    setAudienceOffice(isAdmin ? null : viewerOffice);
    setAudienceDepartment(isAdmin ? null : viewerDepartment);
    setCustomIds([]);
    setEffectiveDate("");
    setPublishMode("now");
    setUploads((prev) => {
  ...
  const submit = async () => {
    ...
    if (uploadingCount > 0)
      return setError("Wait for attachments to finish uploading.");
  ...
  return (
    ...
    >
      ...
      <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Compose office memo</DialogTitle>
          <DialogDescription>
            Publish now, save as draft, or schedule for later. Recipients
            are notified in the bell and (if configured) Slack.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Drafts can be reviewed and published later from the memo
                detail dialog.
              </p>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button
              type="button"
              onClick={submit}
              disabled={submitting || uploadingCount > 0 || !title.trim() || body.trim().length < 3}
            >
              {submitLabel}
            </Button>
          </div>
